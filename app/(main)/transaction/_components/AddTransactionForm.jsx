    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Select onValueChange={value => setValue("accountId", value)} defaultValue={getValues("accountId")}>
            ...
            <SelectContent>
              {accounts.map(account => <SelectItem key={account.id} value={account.id}>{account.name} (${parseFloat(account.balance).toFixed(2)})</SelectItem>)}
              <CreateAccountDrawer>
      ...
      <div className="flex gap-4">
        ...
        <Button type="submit" className="w-full" disabled={transactionLoading}>
          {transactionLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />{editMode ? "Updating..." : "Creating..."}</> : editMode ? "Update Transaction" : "Create Transaction"}
        </Button>
      </div>
    </form>
  )
