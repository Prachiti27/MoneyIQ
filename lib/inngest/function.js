...
export const processRecurringTransaction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    if (!event?.data?.transactionId || !event?.data?.userId) return { error: "Missing required event data" }
    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        const expenses = await db.transaction.aggregate({
          where: { userId: budget.userId, accountId: defaultAccount.id, type: "EXPENSE", date: { gte: startDate } },
          _sum: { amount: true }
        })
        ...
        if (percentageUsed >= 80 && (!budget.lastAlertSent || isNewMonth(new Date(budget.lastAlertSent), new Date()))) {
          await sendEmail({
            ...
            react: EmailTemplate({ userName: budget.user.name, type: "budget-alert", data: { percentageUsed, budgetAmount: parseInt(budgetAmount).toFixed(1), totalExpenses: parseInt(totalExpenses).toFixed(1), accountName: defaultAccount.name } })
          })
